class RostersController < ApplicationController
  ...
  def index
    if params[:search]
      @rosters = Roster.where("first_name LIKE ?", "%#{params[:search]}%")
    else
      @rosters = Roster.all
    end
  end
  ...
  def desc
    @rosters = Roster.order(first_name: :desc)
    render :index
  end

  def asc
    @rosters = Roster.order(first_name: :asc)
    render :index
  end
  ...
  private
    ...
    def roster_params
      params.require(:roster).permit(:first_name, :last_name, :role)
    end
end
